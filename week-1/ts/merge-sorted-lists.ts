type SortedListNode = {
  val: number,
  next: SortedListNode | null
}

/**
 * Definition for singly-linked list. */
function ListNode(val: number = 0, next?: SortedListNode | null) {
...
function mergeLists (list1: SortedListNode | null, list2: SortedListNode | null): SortedListNode | null {
  //set up a dummy linked list (but we won't use this first node)
  let dummy: SortedListNode = new ListNode();
  //in order to reassign the "next" value, we want to keep track of the last node we pointed to
  let prev: SortedListNode = dummy;
